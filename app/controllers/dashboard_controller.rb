class DashboardController < ApplicationController
  ...
  def get_data
   ...
   sectionA = Student.where('classroom_id = ? AND section_id = ?',1,1).map {|a| a.assessments.last}.compact 
   sectionB = Student.where('classroom_id = ? AND section_id = ?',1,2).map {|a| a.assessments.last}.compact
